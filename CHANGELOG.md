# 版本历史
- v3.0.0：与 **AI 审核插件**（`astrbot_plugin_ai_review`）实现双向通信（AstrBot 插件实例互调）。AI 审核通过 `context.get_all_stars()` 发现本插件 `star_cls` 实例并直接调用其接口：`api.add_to_blacklist` 同步拉黑（AI 审核 → 皮梦云）；`service.get_user_data` / `api.check_blacklist` 审核前查询云黑库以加重判定（皮梦云 → AI 审核）。插件未加载时自动跳过（弱依赖），本插件侧接口保持不变、无功能代码改动；同步更新版本与描述
- v2.9.3：修复 `/bl_check` 不指定类型时绕过限流的 Bug（会双次调用 API）；将踢人权限检查失败时的降级默认值改为 `False`，无法确认权限时拒绝踢人以避免误踢；同步模块去除重复的 Token 检查并改用统一日志
- v2.9.2：优化拦截策略：1级（轻微）不进行拦截输出，2~4级（一般/平台/严重）才进行拦截并提示
- v2.9.1：优化文案描述；版本号更新
